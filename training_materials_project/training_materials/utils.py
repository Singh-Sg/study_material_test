...
def count_pdf_pages(pdf_file):
    """
    Count no. of pages in a pdf file.
    """
...
def get_summary(pdf_file, pages):
    """
    Generate summary for the pdf file.
    """
    token = 4096 / pages
...
def generate_system_design(summary):
    """
    Generate system design for the file.
    """
...
def generate_questions(summary):
    """
    Generate single choice based questions.
    """
...
def get_image(summary):
    """
    Generate image related to the topic inside the file.
    """
    summary = summary[1:500]
    response = openai.Image.create(
        prompt=f"Generate image for following text: {summary}", 
        n=1, 
        size="1024x1024"
    )
